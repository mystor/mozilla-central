...
using WindowContextByIdMap = nsRefPtrHashtable<nsUint64HashKey, WindowContext>;
...
already_AddRefed<WindowContext> WindowContext::GetById(
  ...
  return gWindowContexts->Get(aInnerWindowId);
}
...
void WindowContext::CreateFromIPC(IPCInitializer&& aInit) {
  ...
  MOZ_RELEASE_ASSERT(bc && !bc->IsDiscarded());
...
void WindowContext::Init() {
  ...
  auto entry = gWindowContexts->LookupForAdd(mInnerWindowId);
...
bool IPDLParamTraits<dom::WindowContext::IPCInitializer>::Read(
  ...
  if (!ReadIPDLParam(aMessage, aIterator, aActor, &aInit->mInnerWindowId) ||
      !ReadIPDLParam(aMessage, aIterator, aActor, &aInit->mBrowsingContextId) ||
