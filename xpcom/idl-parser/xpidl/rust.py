...
def attributeParamlist(a, getter):
    l = ["%s: %s" % (attributeParamName(a), 
                     a.realtype.rustType(getter and 'out' or 'in'))]
    if a.implicit_jscontext:
        l.insert(0, "cx: *const JSContext")
...
def methodReturnType(m, macro):
    ...
    if m.nostdcall and m.notxpcom:
        return "%s%s" % (macro == "NS_IMETHOD" and "virtual " or "",
                         m.realtype.rustType('in').strip())
    elif m.nostdcall:
        return "%snsresult" % (macro == "NS_IMETHOD" and "virtual " or "")
    elif m.notxpcom:
        return "%s_(%s)" % (macro, m.realtype.rustType('in').strip())
    else:
        return macro
...
def attrAsVTableEntry(m, getter):
    return "%s: unsafe extern \"C\" fn (%s) -> %s" % (attributeNativeName(m, getter),
                                                      attributeParamlist(m, getter),
                                                      attributeReturnType(m, "NS_IMETHOD"))

def methodAsVTableEntry(m):
    return "%s: unsafe extern \"C\" fn (%s) -> %s" % (methodNativeName(m),
                                                      paramlistAsNative(m),
                                                      m.realtype.rustType("in").strip())
...
def write_interface(iface, fd):
    ...
    for member in iface.members:
        if type(member) == xpidl.Attribute:
            entries += "    /* %s */\n" % member.toIDL()
            entries += "    %s,\n" % attrAsVTableEntry(member, True)
            if not member.readonly:
                entries += "    %s,\n" % attrAsVTableEntry(member, False)
            entries += "\n"

        elif type(member) == xpidl.Method:
            entries += "    /* %s */\n" % member.toIDL()
            entries += "    %s,\n\n" % methodAsVTableEntry(member)
...
"""
    ...
    fd.write(str(iface.base) + '\n')
    fd.write(str(dir(iface)))
    ...
    def write_attr_decl(a):
        ...
        if a.infallible:
            fd.write(attr_infallible_tmpl %
                     {'realtype': a.realtype.rustType('in'),
