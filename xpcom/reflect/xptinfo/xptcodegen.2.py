...
#     """A simple wrapper around a write FD which is used in this module"""
#     def __init__(self, fd):
#         self.fd = fd
#         self.indent = 0
#     def write(self, s):
#         # Handle types which want to do their own writing.
#         if hasattr(s, 'write'):
#             return s.write(self)
#         if isinstance(s, bool):
#             s = "true" if s else "false"
#         # Indent newlines
#         self.fd.write(str(s).replace('\n', '\n' + ('  ' * self.indent)))
...
ValDetail.__str__ = lambda self: str(self.value)

# Helper method used by struct(...)
def struct__str__(self):
    s = "{"
...
def struct(name, fields):
    ...
    ty.__str__ = struct__str__
    return ty

    # class Construct(object):
    #     def __init__(self, **values):
    #         assert len(values) == len(fields), "wrong number of values?"
    #         self.values = values
    ...
    #         for f in fields:
    #             fd.write('\n')
    #             fd.write(self.values[f])
    #             fd.write(", // " + f)
    #             if isinstance(f, ValDetail):
    #                 fd.write(" = " + self.values[f].detail)
    #         fd.indent -= 1
    #         fd.write("\n}")
...
class CTable(object):
    ...
    # Add N entries to the table.
    # Takes an itemgen function to avoid unnecessary work.
    def add(self, comment, itemgen, key=None):
        if key is not None: # Check if we're in cache
            idx = self.cache.get(key)
            if idx is not None:
                self.items[idx][1].add(comment)
                return ValDetail(idx, comment)

        idx = len(self.items)
        self.items += ((item, set()) for item in itemgen())
    ...
    def __str__(self):
        s = "const %s nsXPTData::%s = {" % (self.ty, self.name)
        for idx, (it, comments) in enumerate(self.items):
            if len(comments) > 0:
                s += "\n  "
            for comment in comments:
                s += '// %s[%d] = %s\n  ' % (self.name, idx, comment)
            s += indented(str(it))
            s += ','
        return s + '\n};\n\n'
...
def link_to_cpp(interfaces, fd):
    ...
    def lower_type(type, in_=0, out=0, optional=0):
        tag = type['tag']
        d1 = d2 = 0
        ...
        if tag == 'TD_ARRAY':
            ...
            desc = describe_type(type['element'])
            d2 = types.one(desc, lambda: lower_type(type['element']),
                           key=desc).value
            ...
            idx = domobjects.one(type['name'],
                                 lambda: lower_domobject(type),
                                 key=type['name'])
            d1 = idx.value >> 8
            d2 = idx.value & 0xff
    ...
    def lower_method(method, ifacename):
        ...
        return methodinfo(
            ...
            # Flags
            mGetter=int(flags.getter),
            mSetter=int(flags.setter),
            mNotXPCOM=int(flags.notxpcom),
            mHidden=int(flags.hidden),
            mOptArgc=int(flags.optargc),
            mContext=int(flags.jscontext),
            mHasRetval=int(flags.hasretval),
        )
    ...
    def lower_iface(iface):
        ...
        return interfaceinfo(
            ...
            # Flags
            mIsShim=int(isshim),
            mBuiltinClass=int(builtinclass),
            mMainProcessScriptableOnly=int(flags.main_process_only),
            mFunction=int(flags.function),
        )
...
""")
    ...
    fd.write(str(types))
    fd.write(str(params))
    fd.write(str(methods))
    fd.write(str(domobjects))
    fd.write(str(consts))
    fd.write(str(prophooks))
    fd.write(str(strings))
