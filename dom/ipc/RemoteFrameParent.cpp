...
nsresult RemoteFrameParent::Init(const nsString& aPresentationURL,
  ...
  RenderFrame* renderFrame = mTabParent->GetRenderFrame();
  if (NS_WARN_IF(!renderFrame)) {
    MOZ_ASSERT(false, "No RenderFrame");
    return NS_ERROR_FAILURE;
  }

  // Send the newly created layers ID back into content.
  Unused << SendSetLayersId(renderFrame->GetLayersId());
  return NS_OK;
}
...
IPCResult RemoteFrameParent::RecvUpdateDimensions(
  ...
  // HACK: Plz render
  layers::LayersObserverEpoch epoch;
  epoch.mId = 1;
  Unused << mTabParent->SendRenderLayers(true, true, epoch);

  return IPC_OK();
}

IPCResult RemoteFrameParent::RecvSetDocShellIsActive(const bool& aIsActive) {
  Unused << mTabParent->SendSetDocShellIsActive(aIsActive);
  return IPC_OK();
}
