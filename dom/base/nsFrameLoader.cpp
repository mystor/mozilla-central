...
nsresult nsFrameLoader::SwapWithOtherRemoteLoader(
  ...
  // FIXME: Consider supporting FrameLoader swapping for remote frames.
  if (mRemoteFrameChild) {
    return NS_ERROR_NOT_IMPLEMENTED;
  }
...
bool nsFrameLoader::ShouldUseRemoteProcess() {
  ...
  if (XRE_IsContentProcess() &&
      Preferences::GetBool("dom.ipc.fission.testAttribute.enabled", false) &&
